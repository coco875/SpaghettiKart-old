...
ImFont* GameEngine::CreateFontWithSize(float size, std::string fontPath) {
    ...
    if (fontPath == "") {
        ...
        auto file = Ship::Context::GetInstance()->GetResourceManager()->LoadFileProcess(fontPath);
        std::shared_ptr<Ship::Font> fontData = std::static_pointer_cast<Ship::Font>(
            Ship::Context::GetInstance()->GetResourceManager()->GetResourceLoader()->LoadResource(fontPath, file, initData));
