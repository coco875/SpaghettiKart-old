...
void func_800C847C(u8 playerId) {
    if ((gPlayers[playerId].unk_0DE & 1) == 1) {
        if (D_800E9F74[playerId] == 0) {
            ...
            func_800C94A4(playerId);
            if (((gCurrentCourseId == GetChocoMountain()) || (gCurrentCourseId == GetBowsersCastle()) ||
                 (gCurrentCourseId == GetBansheeBoardwalk()) || (gCurrentCourseId == GetYoshiValley()) ||
                 (gCurrentCourseId == GetFrappeSnowland()) || (gCurrentCourseId == GetKoopaTroopaBeach()) ||
                 (gCurrentCourseId == GetRoyalRaceway()) || (gCurrentCourseId == GetSherbetLand()) ||
                 (gCurrentCourseId == GetDkJungle()) || (gCurrentCourseId == GetBigDonut())) &&
        ...
        if (D_800E9F74[playerId] == 1) {
            ...
            if ((gCurrentCourseId == GetKoopaTroopaBeach()) && (D_800EA0EC[playerId] == 0)) {
...
void play_sound2(s32 soundBits) {
    if ((soundBits == SOUND_ACTION_REV_ENGINE) && (gCurrentCourseId == GetDkJungle())) {
        soundBits = SOUND_ARG_LOAD(0x49, 0x00, 0x80, 0x27);
    }

    if ((soundBits == SOUND_ACTION_REV_ENGINE_2) && (gCurrentCourseId == GetDkJungle())) {
