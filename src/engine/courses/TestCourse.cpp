...
void TestCourse::SpawnActors() {
    ...
    Vec3f ballPos = { 0, 60, 0 };
    size_t gridSize = 8;    // Set grid size to 8x8
    float spacing = 100.0f; // Distance between actors
    ...
    for (size_t i = 0; i < 75; i++) {
        ...
        // Update the ball position to spawn in a grid
        ballPos[0] = col * spacing;
        ballPos[2] = row * spacing;

        gWorldInstance.AddActor(std::make_unique<ABall>(ballPos));
    }
}
